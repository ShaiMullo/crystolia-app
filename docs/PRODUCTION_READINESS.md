...
Revised 2026-07-30 (round 2) after an external review: reservation failures and non-transactional order processing were re-classified from deferred to **production blockers** and fixed — see §3. Payment wording now strictly separates a *static payment link* from a *verified payment integration* — see §2.
...
| 2.2 | Card method offered to customers whenever `enabled=true`. **A static HTTPS link is not a payment method**: it gives no confirmation, no amount validation, no webhook. | HARDEN (done, round 2) | High | `utils/paymentOptions.ts`, `CustomerDashboard.tsx`, `paymentStatusService.ts` | Credit card is **not offered at all** and card-order approval is blocked until a verified provider adapter is integrated (`VERIFIED_CARD_PROVIDER_INTEGRATED`). A saved static link is reported separately as `staticLinkUsable` for admin transparency, never as `configured`. | user + creds (provider) |
...
| # | Finding | Class | Severity | Files | Impact / Fix | Needs |
|---|---------|-------|----------|-------|--------------|-------|
| 3.1 | No order-status state machine; concurrent PATCHes could each transition, double-reserve, and double-invoice. | HARDEN (done, round 3) | **Blocker** | `services/orderStatusService.ts`, both order routes | Explicit transition map + an **atomic lock claim** (`statusLockAt` CAS, 2-min TTL) serializes transitions; for stock-tracked orders the claim runs INSIDE the same transaction as everything else. One auto-invoice per order is enforced by a DB-level partial unique index on `Invoice.order`, and that index is an explicit **readiness condition**: verified at boot and on `/api/ready`, and approvals return 503 while it cannot be built (e.g. legacy duplicate invoices) instead of silently continuing. CRM item edits are a single conditional update requiring `status=pending` AND no live transition lock, so an edit can never change lines after a reservation was computed. | — |
| 3.2 | Inventory `applyMovement` was read-modify-write → concurrent approvals could oversell. | HARDEN (done) | High | `services/inventoryService.ts` | Rewritten as conditional atomic `findOneAndUpdate` with availability guard in the filter. Shipping is a single composite update (consume reservation + deduct on-hand atomically), never "release then deduct". | — |
| 3.3 | Reservation failures silently swallowed — order approved + invoiced with no stock held; multi-line orders could reserve partially. | HARDEN (done, round 3) | **Blocker** | `inventoryService.ts`, `orderStatusService.ts`, `db/withTransaction.ts` | Approval **fails closed**: every tracked line must reserve or the order stays `pending` (409, no invoice, no approval notification, `inventoryReserved` set only after full success). Multi-line all-or-nothing is guaranteed by a **required MongoDB transaction** (`runRequiredTransaction`); when transactions are unavailable (standalone mongod) stock-tracked transitions are refused with **503** and an operational message — there is deliberately NO best-effort compensation path claiming atomicity it can't deliver. Untracked-only orders use a safe non-inventory path. **Operational consequences:** (a) production Mongo must be a replica set — Atlas already is; the compose `local-mongo` profile is standalone and will 503 tracked approvals; (b) enter opening stock before going live. | prod-data (enter stock) |
...
| 3.6 | Approval side-effects not transactional (status→stock→invoice→notify were separate unprotected awaits). | HARDEN (done, round 4) | **Blocker** | `orderStatusService.ts`, `db/withTransaction.ts`, `db/indexReadiness.ts`, `db/transactionReadiness.ts` | For stock-tracked orders, **ONE transaction** covers: the lock/CAS claim, inventory summary updates, InventoryMovement writes, the status/flag/timeline commit, and the approval invoice. Any failure — including a failed release or a failed invoice — aborts everything; no notification is sent without a committed result. **Every other production stock writer is transactional too** (manual movements `POST /inventory/movements`, PO receiving) via `runRequiredTransaction` — standalone Mongo gets 503, never partial writes. The sessionless `applyMovement` revert **remains best-effort** (the revert itself can fail, logged CRITICAL) and is a development/seed defense only — no production stock API relies on it. Verified by failure-injection tests on `MongoMemoryReplSet`. | — |
| 3.10 | Nothing verified at deploy time that the runtime can actually run the order workflow (transactions, critical index). | HARDEN (done, round 4) | High | `routes/ready.ts`, `db/transactionReadiness.ts`, `deploy/demo/remote-deploy.sh`, `docker-compose.demo.yml` | `/api/ready` now requires: DB connected + **MongoDB transactions available** (read-only `hello` topology check — replica set/mongos) + unique `Invoice.order` index present. The deploy script polls `/api/ready` and fails loudly (prints the readiness response + backend logs); the compose backend healthcheck gates on `/api/ready` (image HEALTHCHECK keeps `/api/live` for process liveness). A deploy against standalone Mongo or broken invoice data can no longer report success. | — |
| 3.7 | Customer catalog showed availability aggregated across ALL locations while reservations only use `main` — customers could order stock that cannot be reserved. | HARDEN (done, round 2) | High | `services/catalogService.ts` | Display now reads `main` only — the same location the reservation flow uses. Multi-location allocation is documented as a future requirement: display and reservation must always share one allocation policy. | — |
| 3.8 | Admin CRM order creation trusts body-supplied line prices. | DEFER_WITH_REASON | Low | `crmOrders.ts` | Admin-only surface; intentional flexibility (custom pricing). Documented; revisit if agents get order-creation rights. CRM creation directly as `approved` now runs the same fail-closed approval workflow (no bypass); on reservation failure the order is created `pending` with an explicit `approvalError`. | user |
| 3.9 | No email/SMS retry; failed approval emails never auto-resent. | DEFER_WITH_REASON | Medium | `orderNotificationService.ts` | Failures are recorded per-order in the timeline. A retry queue is real infrastructure (jobs/backoff/dedupe) — schedule as its own milestone. | — |
...
2. **Enter real bank details** in Admin → Settings before relying on bank transfer (current values are placeholders). Bank transfer is currently the ONLY payment method that can be offered to customers — credit card stays off until a verified provider integration exists.
2b. **Enter opening stock** (Admin → Inventory) for every stock-tracked product — approval now fails closed, so orders for tracked products cannot be approved while their `main`-location stock is 0/absent. Alternatively disable `stockTrackingEnabled` per product that should not be stock-managed.
3. Demo payment pages are now **disabled by default**: the app 404s them whenever `NODE_ENV=production` (unless explicitly re-enabled with `DEMO_PAYMENT_PAGES_DISABLED=false`), and `docker-compose.demo.yml` defaults the flag to `true`. No server action is required; only a demonstration environment should ever set `false` **in `deploy/demo/.env.demo` on the server** (the compose `--env-file` — setting it in `frontend-client/.env.demo` has NO effect, the compose `environment:` entry overrides the service env_file).
3b. **Owner verification (post-deploy, authenticated):** in Admin → System → Go-Live — (a) verify the saved bank details against the official bank document ("verify bank details": password re-auth + confirmation; stored as a fingerprint, auto-invalidated by any later change); (b) send the one-off test email / admin-SMS verification actions. These are deliberately manual: the system never marks itself verified.
