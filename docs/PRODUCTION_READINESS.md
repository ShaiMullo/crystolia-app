...
| # | Finding | Class | Severity | Files | Impact / Fix | Needs |
|---|---------|-------|----------|-------|--------------|-------|
| 3.1 | No order-status state machine; concurrent PATCHes could each transition, double-reserve, and double-invoice. | HARDEN (done, round 2) | **Blocker** | `services/orderStatusService.ts`, both order routes | Explicit transition map + an **atomic lock claim** (`statusLockAt` CAS, 2-min TTL) serializes transitions: exactly one of N concurrent approvals wins; the rest get 409. A DB-level partial unique index on `Invoice.order` guarantees one auto-invoice per order even across processes. Covered by a concurrent-approval regression test. | — |
...
| 3.3 | Reservation failures silently swallowed — order approved + invoiced with no stock held; multi-line orders could reserve partially. | HARDEN (done, round 2 — was wrongly deferred) | **Blocker** | `inventoryService.ts`, `orderStatusService.ts` | Approval now **fails closed**: every tracked line must reserve or the order stays `pending` (409, no invoice, no approval notification, `inventoryReserved` set only after full success). Multi-line reservation is all-or-nothing — one Mongo transaction on replica sets, sequential-with-compensation on standalone (paired `reservation_compensation` movements keep summary and log in sync). **Operational consequence:** approving orders for stock-tracked products requires Inventory rows with stock (or disabling tracking per product) — enter stock before going live. | prod-data (enter stock) |
...
| 3.6 | Approval side-effects not transactional (status→stock→invoice→notify were separate unprotected awaits). | HARDEN (done, round 2 — was wrongly deferred) | **Blocker** | `orderStatusService.ts`, `db/withTransaction.ts` | Inventory effects run BEFORE the status commit and are all-or-nothing (transaction where supported, compensation otherwise); the status+flag+timeline commit is a single document save; the invoice is DB-unique per order. A crash between steps leaves a recoverable state (lock expires, order keeps previous status or has reservation+approved, invoice retried on next approval). | — |
