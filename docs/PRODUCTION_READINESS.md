...
| 3.6 | Approval side-effects not transactional (status→stock→invoice→notify were separate unprotected awaits). | HARDEN (done, round 3) | **Blocker** | `orderStatusService.ts`, `db/withTransaction.ts`, `db/indexReadiness.ts` | For stock-tracked orders, **ONE transaction** now covers: the lock/CAS claim, inventory summary updates, InventoryMovement writes, the status/flag/timeline commit, and the approval invoice. Any failure — including a failed release or a failed invoice — aborts everything: the order keeps its previous status and reservation state, and no notification is sent (routes notify only after a committed result). A failed release therefore keeps `approved` + `inventoryReserved=true` intact. On the sessionless path (untracked-only orders) the invoice is created BEFORE the status commit with explicit cleanup, and `applyMovement` reverts the summary exactly (from the pre-image, including the release clamp delta) if the movement-log write fails — summary and log cannot diverge. Verified by failure-injection tests on `MongoMemoryReplSet`. | — |
