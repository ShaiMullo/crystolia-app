...
3. After step 1, optionally set `DEMO_PAYMENT_PAGES_DISABLED=true` **in `deploy/demo/.env.demo` on the server** (the compose `--env-file` used by `remote-deploy.sh`) to turn the demo pages into 404s. Note: setting it in `frontend-client/.env.demo` has NO effect — the compose `environment:` entry overrides the service env_file.
